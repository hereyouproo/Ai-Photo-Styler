...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const loadStyles = async () => {
      try {
        ...
        // Migrate old local styles that don't have a category
        const localStyles = parsedLocalStyles.map(s => ({
            ...s,
            category: s.category || 'My Styles'
        }));
  ...
  const handleNewSession = () => {
    ...
    setIsLoading(false);
    setToastMessage("New session started.");
  };
      ...
      if (sourceFiles.length > 0) {
        ...
        if (!candidate || !candidate.content || !candidate.content.parts) {
          ...
          if (candidate?.finishReason) {
              errorMessage = `Generation failed. Reason: ${candidate.finishReason}. Please adjust your prompt or image.`;
          }
        ...
        if (imagePart?.inlineData) {
          ...
          const reason = textPart?.text || "The model did not return an image. This could be due to safety filters.";
        ...
        if (response.generatedImages && response.generatedImages.length > 0 && response.generatedImages[0].image?.imageBytes) {
            ...
            const newImage = `data:image/png;base64,${base64ImageBytes}`;
            const newHistory = [...history.slice(0, historyIndex + 1), newImage];
            setHistory(newHistory);
            setHistoryIndex(newHistory.length - 1);
        } else {
  ...
  const handleOpenStyleEditor = (style: Style | null) => {
    ...
    setIsStyleSelectionOpen(false); // Close selection when opening editor
    setIsStyleEditorOpen(true);
  };
  
  const handleSaveStyle = (style: Style, editingStyle: Style | null) => {
    const isEditing = !!editingStyle;
    if (isEditing) {
      ...
      if (styles.some(s => s.name === style.name)) {
        setToastMessage("A style with this prompt name already exists.");
        return;
      }
  ...
  const CanvasContent = () => {
    ...
    if (currentImageUrl) {
        ...
        const canCompare = sourceImageUrls.length > 0;
        return (
            <div className="relative w-full h-full flex items-center justify-center">
                ...
                {canCompare && (
                    <button
                        ...
                        aria-label="Hold to compare with source image"
                    ...
                    <div className="glassmorphism w-28 h-full rounded-2xl p-2 flex flex-col gap-2">
                        ...
                        <div className="flex-1 overflow-y-auto custom-scrollbar -mr-1 pr-1 space-y-2">
                            {history.map((imageUrl, index) => (
                                <div 
                                    key={`${imageUrl.slice(-10)}-${index}`}
                                    className="relative group w-full aspect-square"
                                >
                                    <button
                                        onClick={() => setHistoryIndex(index)}
                                        className={`w-full h-full rounded-lg flex-shrink-0 overflow-hidden transition-all duration-200 transform hover:scale-105 ${historyIndex === index ? 'ring-2 ring-blue-400' : 'ring-1 ring-white/20'}`}
                                    ...
                                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-lg pointer-events-none">
                                        <button
                                            onClick={(e) => {
                                                e.stopPropagation(); 
                                                handleUseHistoryItemAsSource(index);
                                            }}
                                            title="Use as new source"
                                            className="p-2 rounded-full bg-black/50 hover:bg-white/20 text-white transition-colors pointer-events-auto"
                                            aria-label={`Use history item ${index + 1} as new source`}
                                        >
